...
let loadAllStops osmDataDir =
    let wayStops =
        ...
        |> fun g -> OSM.Transform.Op.wayStopsToOsmOperationalPoints g.elements

    printfn "wayStops: %d" wayStops.Length

    let relationStops =
        readFile<OSMJson> osmDataDir "relation-stations.json"
        |> fun g -> OSM.Transform.Op.relationStopsToOsmOperationalPoints g.elements

    printfn "relationStops: %d" relationStops.Length

    let nodeStops =
        readFile<OSMJson> osmDataDir "node-stations.json"
        |> fun g -> OSM.Transform.Op.nodeStopsToOsmOperationalPoints g.elements
