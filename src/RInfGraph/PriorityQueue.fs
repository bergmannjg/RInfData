// see https://github.com/fsprojects/FSharpx.Collections/blob/master/src/FSharpx.Collections/PriorityQueue.fs
// and https://github.com/fsprojects/FSharpx.Collections/pull/187
...
type Heap<'T when 'T : comparison>(isDescending : bool, length : int, data : HeapData<'T> ) =
    ...
    override this.Equals(other) =
        match other with
        | :? Heap<'T> as y ->
            (this :> System.IEquatable<Heap<'T>>).Equals(y)
        | _ -> false
    ...
    interface System.IEquatable<Heap<'T>> with

        member this.Equals (y : Heap<'T>) =
